...
export function InfinityGauntlet() {
  ...
  useEffect(() => {
    ...
    const featured = ALL_TOOLS.filter((t) => t.isFeatured);
    const verified = ALL_TOOLS.filter((t) => !t.isFeatured && t.verificationLevel !== 'listed-only');
    const pool = [...featured, ...verified, ...ALL_TOOLS];
    let slugs = read<string[]>(sessionStorage, KEYS.picks, []);
    const valid = slugs.filter((s) => pool.some((t) => t.slug === s));
    if (valid.length !== PICK_COUNT) {
      const pickFeatured = [...featured].sort(() => Math.random() - 0.5).slice(0, Math.min(2, featured.length));
      const remainingNeeded = PICK_COUNT - pickFeatured.length;
      const pickOthers = [...verified].sort(() => Math.random() - 0.5).slice(0, remainingNeeded);
      const combined = [...pickFeatured, ...pickOthers];
      slugs = combined.map((t) => t.slug);
  ...
  /* ── Draw a fresh set of random tool stones (featured prioritized) ── */
  const drawFreshPicks = () => {
    ...
    const remainingNeeded = PICK_COUNT - pickFeatured.length;
    const pickOthers = [...verified].sort(() => Math.random() - 0.5).slice(0, remainingNeeded);
    const slugs = [...pickFeatured, ...pickOthers].map((t) => t.slug);
    write(sessionStorage, KEYS.picks, slugs);
    setPicks(slugs.map((s) => ALL_TOOLS.find((t) => t.slug === s)!).filter(Boolean));
  };
