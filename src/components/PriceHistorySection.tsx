...
export function PriceHistorySection({
  ...
  const t = useTranslations('components.priceHistory');
    ...
    return (
      <section className="mt-6 rounded-2xl border border-white/10 bg-surface-1 p-5">
        <h2 className="flex items-center gap-2 text-sm font-bold text-zinc-200">
          <LineChart className="h-4 w-4 text-accent-400" aria-hidden="true" /> {t('heading')}
        </h2>
        <p className="mt-2 text-2xs text-zinc-500">{t('loading')}</p>
      </section>
    );
  }
    ...
    return (
      <section className="mt-6 rounded-2xl border border-white/10 bg-surface-1 p-5">
        ...
        <p className="mt-2 text-2xs leading-relaxed text-zinc-400">
          {t('trackingNew')}{checkedAt ? t('baseline', { date: checkedAt }) : ''}.
          {t('trackingNote')}
        </p>
      </section>
    );
    ...
    <section className="mt-6 rounded-2xl border border-white/10 bg-surface-1 p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {delta !== 0 && (
          ...
          >
            {delta > 0 ? '▲' : '▼'} ${Math.abs(delta).toFixed(2)} {t('since')} {first.noticed_at.slice(0, 10)}
          </span>
        )}
      </div>
