...
export function ToolRecommenderModal({ open, onClose }: RecommenderModalProps) {
  const t = useTranslations('components.toolRecommender');
        ...
        <div className="flex items-center justify-between border-b border-white/10 bg-zinc-950/80 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-2xs text-zinc-400">{t('subtitle')}</p>
            </div>
          </div>
        ...
        <div className="p-6 sm:p-8">
          {step === 1 && (
            <div className="space-y-6">
              <div>
                <span className="text-2xs font-bold uppercase tracking-wider text-accent-400">{t('step', { n: '1' })}</span>
                <h4 className="mt-1 text-xl font-extrabold text-white">{t('taskQuestion')}</h4>
              </div>

              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  { id: 'clipping', titleKey: 'clipping', descKey: 'clippingDesc' },
                  { id: 'faceless', titleKey: 'faceless', descKey: 'facelessDesc' },
                  { id: 'dubbing', titleKey: 'dubbing', descKey: 'dubbingDesc' },
                  { id: 'broll', titleKey: 'broll', descKey: 'brollDesc' },
                  { id: 'podcast', titleKey: 'podcast', descKey: 'podcastDesc' },
                ].map((item) => (
                  <button
                    key={item.id}
                    onClick={() => {
                  ...
                  >
                    <span className="text-sm font-bold">{t(item.titleKey)}</span>
                    <span className="mt-1 text-2xs opacity-70">{t(item.descKey)}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                <span className="text-2xs font-bold uppercase tracking-wider text-accent-400">{t('step', { n: '2' })}</span>
                <h4 className="mt-1 text-xl font-extrabold text-white">{t('budgetQuestion')}</h4>
              </div>

              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  { id: 'free', titleKey: 'free', descKey: 'freeDesc' },
                  { id: 'budget', titleKey: 'budget', descKey: 'budgetDesc' },
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                <span className="text-2xs font-bold uppercase tracking-wider text-accent-400">{t('step', { n: '3' })}</span>
                <h4 className="mt-1 text-xl font-extrabold text-white">{t('priorityQuestion')}</h4>
              </div>

              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  { id: 'quality', titleKey: 'quality', descKey: 'qualityDesc' },
                  { id: 'speed', titleKey: 'speed', descKey: 'speedDesc' },
                  { id: 'ease', titleKey: 'ease', descKey: 'easeDesc' },
                ].map((item) => (
                  <button
                    key={item.id}
                    onClick={() => {
              ...
              <div>
                <div className="flex items-center justify-between">
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 px-3 py-1 text-xs font-bold text-emerald-300">
                    <Trophy className="h-3.5 w-3.5" /> {t('bestVerified')}
                  </span>
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-3">
                        {hasVerifiedScore(topMatch) ? (
                          <span className="rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-bold text-emerald-300">
                            {t('testedSuffix', { score: computeOverall(topMatch.scores!).toFixed(1) })}
                          </span>
                        ) : (
