...
export const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ tools }) => {
  ...
  const getCategoryIcon = (cat: string) => {
    switch (cat) {
      case 'Shorts & Reels': return <Video className="h-4 w-4 text-purple-400" />;
      case 'Voice & Audio': return <Mic className="h-4 w-4 text-pink-400" />;
      case 'Thumbnails & Design': return <ImageIcon className="h-4 w-4 text-blue-400" />;
      default: return <Film className="h-4 w-4 text-indigo-400" />;
    }
  };
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 rounded-full bg-gradient-to-r from-purple-600 via-indigo-600 to-pink-600 px-5 py-3 text-sm font-extrabold text-white shadow-2xl shadow-purple-600/30 transition-all hover:scale-105"
      >
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-6 z-50 w-full max-w-sm sm:max-w-md overflow-hidden rounded-3xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 shadow-2xl">
