...
import { Sparkles, X, Send, Bot, ExternalLink, MessageSquare, Zap } from 'lucide-react';
...
export const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ tools }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: 'bot',
      text: 'سلام! من دستیار هوشمند CreatorAI Hub هستم 🤖. به من بگو چه نوع ویدیویی می‌خواهی بسازی تا بهترین ابزار با تخفیف و لینک مستقیم را بهت پیشنهاد بدم!',
    },
  ]);
  ...
  const handleSend = (userText: string) => {
    ...
    // Smart Local Heuristic matching for solo founder instant response
    const query = userText.toLowerCase();
    let matched: Tool[] = [];

    if (query.includes('short') || query.includes('شورت') || query.includes('tiktok') || query.includes('reels') || query.includes('ریلز') || query.includes('کلیپ')) {
      matched = tools.filter((t) => t.category === 'Shorts & Reels').slice(0, 3);
    } else if (query.includes('voice') || query.includes('صدا') || query.includes('دوبله') || query.includes('audio') || query.includes('پادکست')) {
      matched = tools.filter((t) => t.category === 'Voice & Audio').slice(0, 3);
    } else if (query.includes('thumbnail') || query.includes('تامبنیل') || query.includes('عکس') || query.includes('image') || query.includes('کاور')) {
      matched = tools.filter((t) => t.category === 'Thumbnails & Design').slice(0, 3);
    } else if (query.includes('script') || query.includes('متن') || query.includes('seo') || query.includes('سئو') || query.includes('سناریو')) {
      matched = tools.filter((t) => t.category === 'Script & SEO').slice(0, 3);
    } else {
      // default recommend top featured
      matched = tools.filter((t) => t.isFeatured).slice(0, 3);
    }

    const botReply: Message = {
      sender: 'bot',
      text: `بر اساس درخواستت، بهترین ابزارهای تست‌شده زیر رو بهت پیشنهاد می‌کنم. روی دکمه "بازدید ابزار" کلیک کن تا تستشون کنی:`,
      recommendedTools: matched,
    };
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-6 z-50 w-full max-w-sm sm:max-w-md overflow-hidden rounded-3xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 shadow-2xl transition-colors">
          ...
          <div className="flex items-center justify-between border-b border-zinc-200 dark:border-white/10 bg-purple-600 px-4 py-3 text-white">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-purple-200">پیشنهاد هوشمند ابزار با لینک افیلیت</p>
              </div>
            </div>
            <button
                ...
                {msg.recommendedTools && msg.recommendedTools.length > 0 && (
                  <div className="mt-2 w-full space-y-2">
                    {msg.recommendedTools.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center justify-between gap-3 rounded-2xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-zinc-950 p-2.5 shadow-sm"
                      >
                        <div className="flex items-center gap-2.5">
                          <img
                            src={t.logo}
          ...
          <div className="flex flex-wrap gap-1.5 px-4 pb-2">
            <button
              onClick={() => handleSend('ابزارهای ادیت شورتز و ریلز')}
              className="rounded-full border border-purple-500/30 bg-purple-500/10 px-2.5 py-1 text-[10px] font-bold text-purple-600 dark:text-purple-300 hover:bg-purple-500/20"
            >
              🎬 شورتز و ریلز
            </button>
            <button
              onClick={() => handleSend('ابزارهای صداگذاری و دوبله با هوش مصنوعی')}
              className="rounded-full border border-pink-500/30 bg-pink-500/10 px-2.5 py-1 text-[10px] font-bold text-pink-600 dark:text-pink-300 hover:bg-pink-500/20"
            >
              🎙️ صداگذاری و دوبله
            </button>
            <button
              onClick={() => handleSend('ابزار ساخت تامبنیل یوتیوب')}
              className="rounded-full border border-blue-500/30 bg-blue-500/10 px-2.5 py-1 text-[10px] font-bold text-blue-600 dark:text-blue-300 hover:bg-blue-500/20"
            >
              🖼️ ساخت تامبنیل
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 border-t border-zinc-200 dark:border-white/10 p-3">
            <input
              ...
              placeholder="بپرس چه ابزاری نیاز داری..."
