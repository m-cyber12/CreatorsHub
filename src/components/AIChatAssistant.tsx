...
export const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ tools }) => {
  ...
  const [lang, setLang] = useState<'en' | 'fa'>('en'); // Default English for global traffic
  const [messages, setMessages] = useState<Message[]>([]);
  const [input, setInput] = useState('');

  useEffect(() => {
    // Check if user timezone is Iran/Tehran or browser language is Persian
  ...
  const handleSend = (userText: string) => {
    ...
    if (
      ...
      matched = tools
        .filter((t) => t.category === 'Shorts & Reels')
        .slice(0, 3);
    } else if (
      query.includes('voice') ||
      query.includes('صدا') ||
      query.includes('دوبله') ||
      query.includes('audio') ||
      query.includes('پادکست')
    ) {
      matched = tools
        .filter((t) => t.category === 'Voice & Audio')
        .slice(0, 3);
    } else if (
      query.includes('thumbnail') ||
      query.includes('تامبنیل') ||
      query.includes('عکس') ||
      query.includes('image') ||
      query.includes('کاور')
    ) {
      matched = tools
        .filter((t) => t.category === 'Thumbnails & Design')
        .slice(0, 3);
    } else if (
      ...
      matched = tools
        .filter((t) => t.category === 'Script & SEO')
        .slice(0, 3);
    } else {
        ...
        <div className="fixed bottom-20 right-6 z-50 w-full max-w-sm sm:max-w-md overflow-hidden rounded-3xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 shadow-2xl transition-colors">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-sm font-extrabold">
                  CreatorAI Tool Finder
                </h3>
                <p className="text-[10px] text-purple-200">
                  {lang === 'fa'
                    ? 'پیشنهاد هوشمند ابزار با لینک افیلیت'
                    : 'Smart AI matcher with creator discounts'}
                </p>
              </div>
            </div>
          ...
          <div className="max-h-80 overflow-y-auto p-4 space-y-4 text-xs">
            {messages.map((msg, idx) => (
              ...
              >
                ...
                {msg.recommendedTools &&
                  msg.recommendedTools.length > 0 && (
          ...
          <div className="flex flex-wrap gap-1.5 px-4 pb-2">
            <button
              onClick={() =>
                handleSend(
                  lang === 'fa'
                    ? 'ابزارهای ادیت شورتز و ریلز'
                    : 'Best AI tools for Shorts & Reels'
                )
              }
            ...
            <button
              onClick={() =>
                handleSend(
                  lang === 'fa'
                    ? 'ابزارهای صداگذاری و دوبله با هوش مصنوعی'
                    : 'AI Voice & Dubbing tools'
                )
              }
          ...
          <div className="flex items-center gap-2 border-t border-zinc-200 dark:border-white/10 p-3">
            <input
              ...
              placeholder={
                lang === 'fa'
                  ? 'بپرس چه ابزاری نیاز داری...'
                  : 'Ask which tool you need...'
              }
