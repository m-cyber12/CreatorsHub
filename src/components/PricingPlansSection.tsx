...
export function PricingPlansSection({ tool }: { tool: Tool }) {
  const t = useTranslations('components.pricingPlans');
    ...
    <section className="mt-6 rounded-2xl border border-white/10 bg-surface-1 p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {tool.pricingSourceUrl && (
          ...
          >
            {t('officialSource')} <ExternalLink className="h-3 w-3" aria-hidden="true" />
          </a>
        )}
      </div>
          ...
          <table className="w-full min-w-[560px] border-collapse text-left">
            <thead>
              <tr className="border-b border-white/10 text-2xs uppercase tracking-wide text-zinc-500">
                <th className="py-2 pr-3 font-bold">{t('plan')}</th>
                <th className="py-2 pr-3 font-bold">{t('price')}</th>
                <th className="py-2 pr-3 font-bold">{t('billing')}</th>
                <th className="py-2 font-bold">{t('whatYouGet')}</th>
              </tr>
            </thead>
            <tbody>
