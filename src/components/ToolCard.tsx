...
import { ExternalLink, BadgeCheck, Plus, Check, Play } from 'lucide-react';
...
export function ToolCard({ tool, index = 0 }: ToolCardProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    animationFrameRef.current = requestAnimationFrame(() => {
      ...
      wrap.style.transform = `perspective(1000px) rotateX(${(-py * 5).toFixed(2)}deg) rotateY(${(px * 6).toFixed(2)}deg) translateY(-4px)`;
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              className={`absolute bottom-3 right-3 z-10 inline-flex rounded-lg border px-2 py-0.5 text-2xs font-bold backdrop-blur-md ${pricingClass(
                tool.pricing
              )}`}
            >
          ...
          <div className="flex flex-1 flex-col p-4">
            <div className="mb-2 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="flex items-center gap-1.5 truncate text-base font-bold text-white">
                  <Link
                    href={`/tool/${tool.slug}`}
                    className="truncate transition-colors hover:text-accent-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
                  >
                    {tool.name}
                  </Link>
            ...
            <div className="relative z-10 mt-4 flex items-center gap-2 pt-1">
              <a
                ...
                className="inline-flex min-h-[40px] flex-1 items-center justify-center gap-1.5 rounded-xl bg-accent-500 px-3.5 py-2 text-2xs font-bold text-black transition-all hover:bg-accent-400 hover:shadow-[0_0_20px_rgba(247,201,72,0.4)]"
              >
                Visit site
              ...
              <Link
                href={`/tool/${tool.slug}`}
                className="inline-flex min-h-[40px] items-center justify-center rounded-xl border border-white/10 bg-surface-2 px-3 py-2 text-2xs font-semibold text-zinc-300 transition-colors hover:text-white"
              >
                Details
              ...
              <button
                ...
                className={`inline-flex min-h-[40px] items-center justify-center rounded-xl border px-2.5 py-2 text-2xs font-bold transition-colors ${
                  isCompared
