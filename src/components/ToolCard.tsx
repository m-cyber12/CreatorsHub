...
export const ToolCard: React.FC<ToolCardProps> = ({ tool }) => {
  ...
  const getPricingBadgeColor = (pricing: Tool['pricing']) => {
    switch (pricing) {
      case 'Free': return 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30';
      case 'Freemium': return 'bg-blue-500/15 text-blue-300 border-blue-500/30';
      case 'Free Trial': return 'bg-amber-500/15 text-amber-300 border-amber-500/30';
      case 'Paid': return 'bg-purple-500/15 text-purple-300 border-purple-500/30';
  ...
  const getCategoryIcon = (cat: string) => {
    switch (cat) {
      case 'Shorts & Reels': return <Video className="h-6 w-6 text-purple-400" />;
      case 'Voice & Audio': return <Mic className="h-6 w-6 text-pink-400" />;
      case 'Thumbnails & Design': return <ImageIcon className="h-6 w-6 text-blue-400" />;
      default: return <Film className="h-6 w-6 text-indigo-400" />;
    }
  };
    ...
    <div className="cinematic-card group relative flex flex-col justify-between rounded-3xl p-5 overflow-hidden">
      ...
      <div>
        ...
        <Link href={`/tool/${tool.slug}`} className="block relative overflow-hidden rounded-2xl h-44 w-full bg-zinc-950 mb-5 border border-white/5">
          ...
          <div className="absolute top-3 right-3">
            <span className={`rounded-full border px-2.5 py-1 text-[11px] font-bold shadow-md backdrop-blur-md ${getPricingBadgeColor(tool.pricing)}`}>
              {tool.pricing}
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link href={`/tool/${tool.slug}`} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-zinc-900 p-1.5 shadow-md">
              {!logoError ? (
                <img src={tool.logo} alt={`${tool.name} logo`} onError={() => setLogoError(true)} className="h-full w-full object-cover rounded-lg" />
              ) : (
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {tool.hasFounderBadge && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-purple-500/40 bg-purple-500/15 px-2 py-0.5 text-[10px] font-bold text-purple-300 shadow-sm">
                    <Award className="h-3 w-3 text-purple-400" /> Verified
                  </span>
                )}
              </div>
              <span className="text-xs font-semibold text-zinc-400">{tool.category}</span>
            </div>
          </div>
          {tool.startingPrice && <span className="text-xs font-semibold text-zinc-400">{tool.startingPrice}</span>}
        </div>

        <div className="mt-4">
          <Link href={`/tool/${tool.slug}`}>
            <p className="text-sm font-bold text-purple-300/95 leading-snug hover:underline">{tool.tagline}</p>
          </Link>
          <p className="mt-2 text-xs leading-relaxed text-zinc-400 line-clamp-2">{tool.description}</p>
        </div>

        <div className="mt-4 flex flex-wrap gap-1.5">
          {tool.tags.map((tag) => (
            <span key={tag} className="rounded-lg border border-white/5 bg-zinc-900/80 px-2 py-1 text-[11px] font-medium text-zinc-300">
              #{tag}
            </span>
          ))}
        </div>
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-4">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-sm font-extrabold text-white">{tool.rating.toFixed(1)}</span>
          <span className="text-xs font-medium text-zinc-500">({tool.reviewsCount})</span>
        </div>
