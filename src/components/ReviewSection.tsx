    ...
    <section className="mt-12">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-purple-500/30 bg-purple-500/10 px-4 py-2.5 text-xs font-bold text-purple-300 hover:bg-purple-500/20 transition-colors"
        >
      ...
      {formOpen && (
        <form onSubmit={submit} className="mb-6 rounded-3xl border border-purple-500/20 bg-zinc-900/60 p-6 space-y-4">
          {!user && (
            <p className="text-[11px] text-zinc-500">
              Tip: <Link href="/login" className="text-purple-400 underline">sign in</Link> to attach reviews to your account — or post as a guest below.
            </p>
          )}
          <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <input
              ...
              className="rounded-xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-white placeholder-zinc-500 focus:border-purple-500 focus:outline-none"
            />
            <input
          ...
          <textarea
            ...
            className="w-full rounded-xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-white placeholder-zinc-500 focus:border-purple-500 focus:outline-none"
          />
          ...
          {status === 'success' ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-3 text-sm font-bold text-white hover:opacity-90 disabled:opacity-50 transition-opacity"
            >
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-semibold transition-colors ${
                    helpfulVoted.includes(r.id) ? 'text-purple-400' : 'text-zinc-500 hover:text-white'
                  }`}
                >
