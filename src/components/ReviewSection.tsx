...
export function ReviewSection({ toolSlug, toolName }: { toolSlug: string; toolName: string }) {
  const t = useTranslations('components.reviewSection');
  const { user } = useAuth();
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            {avg ? t('avgLine', { avg, count: String(reviews.length) }) : t('beFirst', { name: toolName })}
          </p>
        </div>
        <button
        ...
        >
          <MessageSquarePlus className="h-4 w-4" /> {t('writeReview')}
        </button>
      </div>
        ...
        <form onSubmit={submit} className="mb-6 rounded-3xl border border-accent-500/20 bg-zinc-900/60 p-6 space-y-4">
          {!user && (
            <p className="text-2xs text-zinc-500">
              {t('tip')} <Link href="/login" className="text-accent-400 underline">{t('signIn')}</Link> {t('tipBody')}
            </p>
          )}
          <div>
            <span id="rating-label" className="mb-1.5 block text-xs font-bold text-zinc-300">{t('yourRating')}</span>
            <div className="flex items-center gap-1" role="group" aria-labelledby="rating-label">
              {[1, 2, 3, 4, 5].map((s) => (
                <button
                  ...
                  aria-label={t('starsAria', { count: String(s) })}
                >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input
              ...
              placeholder={user?.email ? t('displayName', { name: user.email.split('@')[0] }) : t('namePlaceholder')}
              className="rounded-xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-white placeholder-zinc-500 focus:border-accent-500 focus:outline-none"
            />
          </div>
          <textarea
          ...
          {status === 'success' ? (
            <p className="inline-flex items-center gap-2 text-sm font-bold text-emerald-400"><CheckCircle2 className="h-4 w-4" /> {t('submitted')}</p>
          ) : (
            <button
            ...
            >
              {status === 'submitting' && <Loader2 className="h-4 w-4 animate-spin" />} {t('submitReview')}
            </button>
          )}
        </form>
      )}
          ...
          {reviews.map((r) => (
            <article key={r.id} className="rounded-2xl border border-white/10 bg-zinc-900/50 p-5">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                >
                  <ThumbsUp className="h-3 w-3" /> {t('helpful', { count: String(r.helpful_count || 0) })}
                </button>
              </div>
            </article>
          ))}
