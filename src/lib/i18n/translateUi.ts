...
import { translateFields, hashSource, resolveProvider } from './engine';
...
export async function translateUiMessages(
  ...
  // Keys that need (re)translation: en changed since last run, missing, or
  // still holding English fallback text (a locale value identical to the
  // English source means it was never really translated).
  ...
  for (const [key, enText] of Object.entries(enFlat)) {
    ...
    if (cache[key] === hashSource(enText) && existing && existing !== enText) {
      finalFlat[key] = existing;
      skipped++;
    } else if (!enText.trim()) {
  ...
  // Provider-agnostic (v3.3): use whichever backend is configured — Gemini
  // (free tier) by default, or OpenAI / OpenRouter. Falls back gracefully to
  // keeping English when no engine is configured at all.
  const provider = await resolveProvider();
  if (!provider) {
    // No engine configured: keep every missing key as its English source so
    // the JSON stays complete (next-intl requires full key parity).
    for (const [key, text] of Object.entries(toTranslate)) finalFlat[key] = text;
    writeFileSync(localeFile, JSON.stringify(unflatten(finalFlat), null, 2) + '\n', 'utf8');
    return { translated: 0, skipped: Object.keys(toTranslate).length };
  }
  ...
  for (let i = 0; i < keys.length; i += BATCH) {
    ...
    if (res) Object.assign(translated, res);
    // Keys the engine skipped keep their previous text, or the English source.
  ...
  // Keys that were neither previously translated nor engine-translated fall
  // back to the English source (keeps every locale file complete).
  for (const [key, text] of Object.entries(toTranslate)) {
    if (!finalFlat[key]) finalFlat[key] = text;
  }
