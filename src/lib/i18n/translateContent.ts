 ...
 *
 * v3.3 quota balance: headlines + excerpts are translated for up to
 * `maxItems` items (cheap), while the FULL article body is only translated
 * for the newest `contentMax` items. Anything else is picked up on demand
 * (lib/newsTranslate.ts → news detail page) or by the daily cron.
 */
export async function autoTranslateNews(
  items: NewsItem[],
  opts: { maxItems?: number; contentMax?: number } = {}
): Promise<number> {
  if (!(await isEngineConfiguredFull()) || items.length === 0) return 0;
  const maxItems = opts.maxItems ?? 8;
  const contentMax = opts.contentMax ?? 3;
  ...
  for (const locale of TRANSLATED_LOCALES) {
    for (let i = 0; i < entities.length; i++) {
      ...
      for (const [f, t] of Object.entries(e.fields)) {
        if (i >= contentMax && f === 'content') continue;
