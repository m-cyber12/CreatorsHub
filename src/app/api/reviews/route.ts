...
export async function POST(request: Request) {
  ...
  try {
    const { tool_slug, rating, title, body, author_name, user_id } = await request.json();
    ...
    if (!supabase) {
      ...
      return NextResponse.json({
        success: true,
        review: {
          id: `mock-${Date.now()}`, tool_slug, rating: r, title: title.trim(),
          body: body.trim(), author_name: safeName, helpful_count: 0, created_at: new Date().toISOString(),
        },
        message: 'Mock mode — connect Supabase to persist reviews.',
      }, { status: 201 });
    }
