...
export default function AdminPage() {
  ...
  // Site Settings (Content + Design + Layout + SEO)
  ...
  // Modal Editing State for an existing tool
  ...
  // Quick Inline Update (e.g. affiliate URL, switches)
  ...
  const handleCreateTool = async (e: React.FormEvent) => {
    ...
    setSaveStatus('saving');
    try {
      const formattedTags = newTool.tags
        .split(',')
        .map((t) => t.trim());
      const res = await fetch('/api/tools', {
        method: 'POST',
  ...
  const handleExportJson = () => {
    ...
    downloadAnchor.setAttribute('download', `creator-ai-hub-backup-${Date.now()}.json`);
  ...
  const handleImportJson = async () => {
    ...
    try {
      ...
      setBackupMsg(`${importedCount} ابزار با موفقیت از فایل JSON وارد دیتابیس شد!`);
      setImportJsonText('');
      fetchAllData();
    } catch (err) {
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-zinc-900/90 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-purple-400">
                Full access: Content, Design, Layout, Affiliates, CRUD &amp; Backups
              </p>
            </div>
          </div>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-1.5 border-t border-white/5 px-4 pt-3 pb-2.5 sm:px-6 lg:px-8">
          ...
          >
            ...
            <span>۲. طراحی، رنگ و چیدمان</span>
          </button>
          ...
          >
            ...
            <span>۳. مدیریت ابزارها و افیلیت ({totalTools})</span>
          </button>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {saveStatus === 'saved' && (
          <div className="mb-6 flex items-center gap-2.5 rounded-2xl border border-emerald-500/30 bg-emerald-500/15 p-4 text-sm font-bold text-emerald-300">
            ...
            <span>تغییرات با موفقیت در دیتابیس سوپابیس ذخیره شد و روی سایت اعمال شد!</span>
          </div>
        )}
          ...
          <form onSubmit={handleSaveAllSettings} className="space-y-6">
            <div className="rounded-3xl border border-white/10 bg-zinc-900 p-6 shadow-xl sm:p-8">
              ...
              <p className="mt-1 text-xs text-zinc-400">
                تغییر زنده تیتر، شعار، بنرها و متادیتای سئو بدون ۱ خط کدنویسی
              </p>

              <div className="mt-6 space-y-5">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">
                    Hero Badge Text (بج درخشان بالای سایت)
                  </label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 border-t border-white/5 pt-5">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">
                      Announcement Banner Title (بنر Founder Flywheel)
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">
                      Footer Copyright Text (متن فوتر)
                    </label>
                    <input
                      type="text"
        ...
        {/* TAB 2: DESIGN & LAYOUT CONTROLS */}
        {activeTab === 'design' && (
          <form onSubmit={handleSaveAllSettings} className="space-y-6">
            <div className="space-y-6 rounded-3xl border border-white/10 bg-zinc-900 p-6 shadow-xl sm:p-8">
              <div>
                <h2 className="text-lg font-extrabold text-white">
                  تنظیمات طراحی، رنگ‌بندی و چیدمان سایت (Design &amp; Theme Controls)
                </h2>
                <p className="mt-1 text-xs text-zinc-400">
                  تغییر زنده تم رنگی، تعداد ستون‌های چیدمان، استایل کارت‌ها و انیمیشن‌های ۳ بعدی
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
                  ...
                  >
                    ...
                    <option value="rose">🔴 رز نئونی (Neon Rose)</option>
                  </select>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
                  ...
                  >
                    <option value="3d-glass">
                      ✨ شیشه‌ای ۳ بعدی با هاور نئونی (MotionSites)
                    </option>
                ...
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
                  ...
                  >
                    ...
                    <option value="static">⚡ استاتیک (بدون انیمیشن برای سرعت بالا)</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-white/10 bg-zinc-900 p-6 shadow-xl sm:p-8">
              <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <h2 className="text-lg font-extrabold text-white">
                    مدیریت ابزارها، لینک‌های افیلیت و ویرایش کامل (Full Tools Manager)
                  </h2>
                  <p className="mt-1 text-xs text-zinc-400">
                    ویرایش فوری لینک افیلیت درون جدول، یا زدن دکمه «Edit» برای ویرایش تمام فیلدهای ابزار
                  </p>
                </div>
                ...
                <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/10 bg-zinc-900 p-6 shadow-2xl">
                  ...
                  <form onSubmit={handleSaveEditedTool} className="mt-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">
                          Cover Image URL (عکس ۱۶:۹)
                        </label>
                        <input
                          type="url"
                          value={editingTool.cover_image || ''}
                          onChange={(e) =>
                            setEditingTool({
                              ...editingTool,
                      ...
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">
                          Tags (با کاما جدا کنید)
                        </label>
                        <input
                          type="text"
                          value={
                            Array.isArray(editingTool.tags)
                              ? editingTool.tags.join(', ')
                              : editingTool.tags || ''
                          }
                          onChange={(e) =>
                            setEditingTool({
