        ...
        {deals.length > 0 && (
          <section className="mb-14">
            <h2 className="mb-5 flex items-center gap-2 text-xl font-bold">
              <BadgePercent className="h-5 w-5 text-accent-400" aria-hidden="true" />
              {t('verifiedDeals')}
            </h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {deals.map((deal) => (
                <a
                  key={deal.id}
                  href={deal.url || (deal.toolSlug ? `/go/${deal.toolSlug}` : '/deals')}
                  target="_blank"
                  rel="noopener noreferrer nofollow"
                  className="group rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5 transition-colors hover:border-emerald-500/50"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="text-sm font-bold text-white group-hover:text-emerald-300">{deal.title}</h3>
                    {deal.code && (
                      <span className="rounded-lg border border-emerald-500/40 bg-emerald-500/15 px-2.5 py-1 font-mono text-2xs font-black tracking-wider text-emerald-300">
                        {deal.code}
                      </span>
                    )}
                  </div>
                  {deal.discount && <p className="mt-1 text-2xs font-bold text-emerald-400">{deal.discount}</p>}
                  {deal.description && <p className="mt-1.5 text-2xs leading-relaxed text-zinc-400">{deal.description}</p>}
                  <span className="mt-3 inline-flex items-center gap-1 text-2xs font-bold text-accent-400">
                    {t('getDeal')} <ExternalLink className="h-3 w-3" aria-hidden="true" />
                  </span>
                </a>
              ))}
            </div>
