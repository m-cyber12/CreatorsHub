...
export default async function CreatorPicksPage({ params }: { params: LocaleParams }) {
  ...
  const priceChecked = await localizeTools(
    effectiveTools.filter((tool) => tool.verificationLevel === 'pricing-verified').slice(0, 6),
    locale
  );
  const newlyCatalogued = await localizeTools(effectiveTools.filter((tool) => tool.isNew).slice(0, 6), locale);
  return (
