...
export function SubmitForm() {
    ...
    return (
      <div className="rounded-3xl border border-emerald-500/30 bg-emerald-500/10 p-10 text-center">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Our editorial team will test <span className="font-semibold text-white">{formData.name}</span> and email{' '}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-purple-500/20 bg-purple-500/5 p-4">
        <input
        ...
        <span className="text-xs leading-relaxed text-zinc-400">
          <span className="inline-flex items-center gap-1 font-bold text-purple-300"><ShieldCheck className="h-3.5 w-3.5" /> Verified Founder Badge</span>
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 py-3.5 text-sm font-bold text-white hover:opacity-90 disabled:opacity-50 transition-opacity"
      >
        {status === 'submitting' && <Loader2 className="h-4 w-4 animate-spin" />}
        Submit for Editorial Review
      </button>
      <p className="text-center text-[10px] text-zinc-600">
        We test every submission by hand. Spam, dead links, and off-niche tools are rejected.
      </p>
    </form>
  );
}
