...
4. Verify — every row should be `SELECT`-only for anon/authenticated:
...
```bash
npm install     # new deps: @vercel/analytics, @vercel/speed-insights, server-only, eslint, prettier
npm run verify  # typecheck + lint + data integrity
npm run build
```

All three must pass. `npm run verify` also runs in CI on every push
